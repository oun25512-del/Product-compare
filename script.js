/* ========== DATA ========== */
const P=[
 {id:1,n:'iPhone 15 Pro Max',b:'Apple',c:'สมาร์ทโฟน',e:'📱',p:49900,o:52900,r:4.8,rv:2847,iN:false,iS:true,iU:false,
 ...
 {id:2,n:'Samsung Galaxy S24 Ultra',b:'Samsung',c:'สมาร์ทโฟน',e:'📲',p:42900,o:44900,r:4.7,rv:1923,iN:true,iS:true,iU:false,
 ...
 {id:3,n:'Google Pixel 8 Pro',b:'Google',c:'สมาร์ทโฟน',e:'🤖',p:32900,o:32900,r:4.6,rv:876,iN:true,iS:false,iU:false,
 ...
 {id:4,n:'MacBook Pro 14" M3 Pro',b:'Apple',c:'โน้ตบุ๊ก',e:'💻',p:79900,o:84900,r:4.9,rv:1234,iN:true,iS:true,iU:false,
 ...
 {id:5,n:'Dell XPS 15 OLED',b:'Dell',c:'โน้ตบุ๊ก',e:'🖥️',p:59900,o:64900,r:4.5,rv:654,iN:false,iS:true,iU:false,
 ...
 {id:6,n:'ASUS ROG Zephyrus G16',b:'ASUS',c:'โน้ตบุ๊ก',e:'🎮',p:69900,o:74900,r:4.7,rv:432,iN:true,iS:true,iU:false,
 ...
 {id:7,n:'Sony WH-1000XM5',b:'Sony',c:'หูฟัง',e:'🎧',p:11900,o:13900,r:4.8,rv:3241,iN:false,iS:true,iU:false,
 ...
 {id:8,n:'Apple AirPods Pro 2',b:'Apple',c:'หูฟัง',e:'🎵',p:9990,o:9990,r:4.7,rv:5612,iN:false,iS:false,iU:false,
 ...
 {id:9,n:'Apple Watch Series 9',b:'Apple',c:'สมาร์ทวอทช์',e:'⌚',p:14900,o:16900,r:4.8,rv:2103,iN:true,iS:true,iU:false,
 ...
 {id:10,n:'Samsung Galaxy Watch 6 Classic',b:'Samsung',c:'สมาร์ทวอทช์',e:'🕐',p:12900,o:14500,r:4.6,rv:987,iN:true,iS:true,iU:false,
 ...
 {id:11,n:'iPad Pro 12.9" M2',b:'Apple',c:'แท็บเล็ต',e:'📋',p:43900,o:46900,r:4.8,rv:1567,iN:false,iS:true,iU:false,
 ...
 {id:12,n:'Samsung Galaxy Tab S9+',b:'Samsung',c:'แท็บเล็ต',e:'📱',p:36900,o:39900,r:4.6,rv:892,iN:true,iS:true,iU:false,
...
let currentImgB64=null, currentImgType=null, aiData=null, nextId=100;
// ✏️ ใส่ Anthropic API Key ของคุณที่นี่
const API_KEY = 'sk-ant-api03-zqQ...ogAA';
let apiKey = API_KEY;
...
function init(){
  ...
  rP();uSt();
  document.getElementById('aiBtn').disabled=true; // เปิดใช้ได้เมื่อกรอกชื่อสินค้า
}



function checkAIBtn(){
  document.getElementById('aiBtn').disabled=!document.getElementById('fName').value.trim();
}
...
function loadImg(file){
  ...
  if(file.size>10*1024*1024){ts('⚠️','ไฟล์ใหญ่เกิน 10MB');return;}
  const r=new FileReader();
  r.onload=ev=>{
    const full=ev.target.result;
    currentImgB64=full.split(',')[1];
    currentImgType=file.type;
    document.getElementById('imgPreview').src=full;
    document.getElementById('imgPreviewWrap').style.display='block';
    document.getElementById('dropZone').style.display='none';
    ts('🖼️','โหลดรูปเรียบร้อย');
  };
  r.readAsDataURL(file);
}
function removeImg(){
  currentImgB64=null;currentImgType=null;
...
async function analyzeAI(){
  ...
  const cat=document.getElementById('fCat').value.trim()||'ไม่ระบุ';
  const price=document.getElementById('fPrice').value||'0';
  if(!name){ts('⚠️','กรุณาใส่ชื่อสินค้า');return;}

  const btn=document.getElementById('aiBtn');
  btn.disabled=true;
  document.getElementById('aiBtnIcon').className='spin';
  document.getElementById('aiBtnIcon').textContent='⟳';
  document.getElementById('aiResult').classList.remove('show');
  document.getElementById('addBtn').classList.remove('show');
  document.getElementById('tipBox').style.display='none';

  const prompt=`คุณเป็นผู้เชี่ยวชาญวิเคราะห์สินค้า วิเคราะห์สินค้านี้แล้วตอบเป็น JSON เท่านั้น ห้ามมีข้อความอื่น

สินค้า: ${name}
แบรนด์: ${brand}
หมวดหมู่: ${cat}
ราคา: ${price} บาท
${currentImgB64?'(มีรูปประกอบ - วิเคราะห์จากรูปด้วย)':'(ไม่มีรูป - วิเคราะห์จากชื่อและข้อมูลที่ให้)'}

ตอบ JSON นี้เท่านั้น (ภาษาไทย):
{
  "description": "คำอธิบายสินค้า 1-2 ประโยค",
  "tags": ["tag1","tag2","tag3"],
  "specs": {"สเปกที่1":"ค่า","สเปกที่2":"ค่า","สเปกที่3":"ค่า","สเปกที่4":"ค่า","สเปกที่5":"ค่า"},
  "scores": {"ด้านที่1":85,"ด้านที่2":90,"ด้านที่3":75,"ด้านที่4":80,"ด้านที่5":70},
  "rating": 4.3,
  "emoji": "🛍️"
}

หมายเหตุ: scores ต้องมี 5 ด้านที่เหมาะกับประเภทสินค้านั้น คะแนน 0-100 จริงๆ อย่าให้ทุกอย่าง 90+`;

  const messages=[{role:'user',content:[]}];
  if(currentImgB64){
    messages[0].content.push({type:'image',source:{type:'base64',media_type:currentImgType,data:currentImgB64}});
  }
  ...
  try{
    ...
    ts('❌','เกิดข้อผิดพลาด: '+err.message);
    document.getElementById('tipBox').style.display='block';
    console.error(err);
  }finally{
    btn.disabled=false;
    document.getElementById('aiBtnIcon').className='';
    document.getElementById('aiBtnIcon').textContent='✨';
    checkAIBtn();
  }
}
...
function renderAIResult(d){
  ...
  document.getElementById('aiScores').innerHTML=Object.entries(scores).map(([k,v])=>`
    <div class="score-row">
  ...
  const specs=d.specs||{};
  document.getElementById('aiSpecs').innerHTML=Object.entries(specs).map(([k,v])=>`
...
function addProduct(){
  ...
  if(!name){ts('⚠️','กรุณาใส่ชื่อสินค้า');return;}
  if(!aiData){ts('⚠️','กรุณาวิเคราะห์ด้วย AI ก่อน');return;}
  ...
  const newProduct={
    ...
    e:aiData.emoji||'📦',
    p:price,
    o:oprice>price?oprice:price,
    r:aiData.rating||4.0,
    rv:0,
    iN:true,
    iS:oprice>price,
    iU:true,
    img:currentImgB64?`data:${currentImgType};base64,${currentImgB64}`:null,
    sp:aiData.specs||{},
    tg:aiData.tags||[],
    ds:aiData.description||'',
    sc:aiData.scores||{}
  };
  P.push(newProduct);
...
function rC(p){
  ...
  const badge=p.iU?'<div class="bai">🤖 ของฉัน</div>':p.iN?'<div class="bn2">ใหม่</div>':p.iS?`<div class="bs">-${dc}%</div>`:'';
...
function bCT(){
  ...
  let h=`<table class="ctb2"><thead><tr><th class="cl">สินค้า</th>${pr.map(p=>{
    const thumb=p.img?`<img src="${p.img}" style="width:40px;height:40px;border-radius:8px;object-fit:cover;display:block;margin:0 auto 5px">`:`<span class="pe">${p.e}</span>`;
    return`<th class="cp2 ${p.p===mp?'hl':''}"><div class="ph">${thumb}<div class="pbr">${p.b}</div><div class="pn">${p.n}${p.iU?'<span class="user-badge">ของฉัน</span>':''}</div><div class="pp">฿${p.p.toLocaleString()}</div>${p.iS?`<div style="font-size:10px;color:var(--t3);text-decoration:line-through">฿${p.o.toLocaleString()}</div>`:''} ${p.p===mp?'<div class="bb">💰 ราคาดีที่สุด</div>':''}</div></th>`;
  }).join('')}</tr></thead><tbody>`;
...
function oD(id){
  ...
  const imgEl=p.img?`<img src="${p.img}" alt="${p.n}" style="width:100%;height:100%;object-fit:cover">`:`${p.e}`;
  document.getElementById('db').innerHTML=`<div class="di">${imgEl}</div><div>
    <div class="dbrnd">${p.b} · ${p.c}${p.iU?' · <span style="color:var(--ac3)">สินค้าของฉัน 🤖</span>':''}</div>
  ...
  l.innerHTML=hist.map(h=>{
    ...
    return`<div style="background:var(--s);border:1px solid var(--bd);border-radius:var(--r);padding:16px;margin-bottom:12px">
      ...
      <div style="display:flex;gap:9px;flex-wrap:wrap">${pr.map(p=>{
        const thumb=p.img?`<img src="${p.img}" style="width:28px;height:28px;border-radius:5px;object-fit:cover">`:`<span style="font-size:18px">${p.e}</span>`;
        return`<div style="display:flex;align-items:center;gap:7px;background:var(--s2);border:1px solid var(--bd);border-radius:9px;padding:7px 12px">${thumb}<div><div style="font-size:10px;color:var(--t3)">${p.b}</div><div style="font-size:12px;font-weight:600">${p.n}</div></div></div>`;
      }).join('')}</div>
    </div>`;
  }).join('');
}
...
document.getElementById('fName').addEventListener('input',checkAIBtn);
